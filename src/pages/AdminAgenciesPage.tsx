...
const AdminAgenciesPage: React.FC = () => {
  ...
  const [newAgency, setNewAgency] = useState({ name: '', logo: '', description: '' });
  ...
  const updateMutation = useMutation({
    mutationFn: async (agency: Agency) => {
      return agencies.map(a => a.id === agency.id ? { ...a, ...agency } : a);
    },
    onSuccess: (data) => queryClient.setQueryData(['agencies'], data)
  });
  ...
  const handleEdit = (agency: Agency) => {
    ...
    setEditData({ name: agency.name, logo: agency.logo, description: agency.description || '' });
  };

  const handleUpdate = (e: React.FormEvent) => {
    e.preventDefault();
    if (editingId) {
      updateMutation.mutate({ ...agencies.find(a => a.id === editingId)!, ...editData });
      setEditingId(null);
    }
  };
                        ...
                        <tr key={agency.id} className="border-b">
                          ...
                          <td className="p-2 space-x-2">
                            {editingId === agency.id ? (
                              ...
                              <>
                                <Button size="sm" variant="outline" onClick={() => handleEdit(agency)}>Edit</Button>
                                <Button size="sm" variant="destructive" onClick={() => deleteMutation.mutate(agency.id)}>Delete</Button>
                              </>
                            )}
                          </td>
                        </tr>
